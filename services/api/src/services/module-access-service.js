...
class ModuleAccessService {
  ...
  async isActive(tenantId = DEFAULT_TENANT_ID, moduleCode) {
    ...
    const subscriptionActive = await getSaasBillingService().isSubscriptionActive(tenantId);
  ...
  async assertActive(tenantId, moduleCode) {
    if (isMultiTenantEnabled()) {
      const subActive = await getSaasBillingService().isSubscriptionActive(tenantId);
      if (!subActive) {
